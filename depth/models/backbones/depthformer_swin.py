...
def psudo_color(Array2d,colormap = 'plasma', percent = 98):
    array2d = Array2d.copy()
    array2d = 1/array2d
    cm = get_cmap(colormap)
    # normalizer = np.percentile(array2d,percent)
    # normalizer = 1 / 5.0
    # array2d /= (normalizer + 1e-5)
    array2d /= array2d.max()
    # embed()
    # exit()np.clip(array2d, 0.9, 1.0)
    array3d = (cm(array2d)[:, :, :3]*255)[:,:,::-1] #[H,W,3]
...
class AttnHeadPEFPN(nn.Module):
    def __init__(self):
        super(AttnHeadPEFPN, self).__init__()
        self.conv_list = nn.ModuleList()
        self.conv_list1 = nn.ModuleList()
        self.input_list = [64,256,512,1024,2048]
        for i in range(len(self.input_list)):
            self.conv_list.append(
                ConvModule(
                    in_channels=self.input_list[i],
                    out_channels=1,
                    kernel_size=1,
                    stride=1,
                    padding=0,
    ...
    def forward(self,x):
        ...
        for index, feat in enumerate(x):
            ...
            temp_feat = self.conv_list1[index](temp_feat)
            temp_feat = self.sigmoid(temp_feat)
            outs.append(temp_feat)
        
        return outs


class AttnHeadPE(nn.Module):
    def __init__(self):
        super(AttnHeadPE, self).__init__()
        self.conv0 = nn.Conv2d(2048, 1024, kernel_size=1, stride=1, padding=0)
    ...
    def load_pretrain(self,pth_path=None):
        if not pth_path is None:
            pretrained_state = torch.load(pth_path, map_location='cpu')
            model_dict = self.state_dict()
            for name, param in pretrained_state.items():
                name = name.replace('AttnHeadPE.','')
    ...
    def forward(self,x):
        ...
        x0 = self.conv0(x0)
        up_x0 = self._upsample_add(x0,x1)
        x1 = x1+up_x0

        x1 = self.conv1(x1)
        up_x1 = self._upsample_add(x1,x2)
        x2 = x2+up_x1

        x2 = self.conv2(x2)
        up_x2 = self._upsample_add(x2,x3)
        x3 = x3+up_x2

        x3 = self.conv3(x3)
        up_x3 = self._upsample_add(x3,x4)
        x4 = x4+up_x3
...
class DepthFormerSwin(BaseModule):
    ...
    def __init__(self,
        ...
        if self.USE_PARAM_PE:
            self.Param_pe1 = build_conv_layer(
                self.conv_cfg,
                4,
                1,
                kernel_size=1,
                stride=1,
                bias=True)
            self.Param_pe_RELU1 = nn.ReLU(inplace=True)

            self.Param_pe2 = build_conv_layer(
                self.conv_cfg,
                1,
                1,
                kernel_size=1,
            ...
            self.Param_pe_sigmoid = nn.Sigmoid()

            nn.init.constant(self.Param_pe1.weight, 1.0)
            nn.init.constant(self.Param_pe1.bias, 0.0)

            nn.init.constant(self.Param_pe2.weight, 1.0)
            nn.init.constant(self.Param_pe2.bias, 0.0)
    ...
    def forward(self, x_ori):
        ...
        if self.USEPE:
            ...
            conv_stem = self.conv_stem(x_ori)
            outs.append(conv_stem)
        
        if self.USE_PARAM_PE:
            t = self.Param_pe1(x.clone())
            t = self.Param_pe_RELU1(t)
            t = self.Param_pe2(t)
            t = self.Param_pe_sigmoid(t)
            x[:,3,:,:] = t[:,0,:,:]*x[:,3,:,:].clone()
