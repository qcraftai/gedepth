    ...
    def __call__(self, results):
        ...
        if 'depth_gt' in results:
            ...
            results['depth_gt'] = DC(
                ...
                stack=True)
        if 'normals_gt' in results:
            normals = results['normals_gt']
            normals = np.ascontiguousarray(normals.transpose(2, 0, 1))
            results['normals_gt'] = DC(to_tensor(normals), stack=True)
        return results
